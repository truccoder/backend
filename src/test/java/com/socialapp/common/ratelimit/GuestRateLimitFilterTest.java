...
class GuestRateLimitFilterTest {
  ...
  class DoFilterInternal {
    ...
    @Test
    @DisplayName("lets the request through and sets the TTL on the first request of a window")
    void firstRequestSetsTtl() throws Exception {
      // Given
      ...
      verify(redisTemplate).expire("ratelimit:guest:" + CLIENT_IP, Duration.ofMinutes(1));
    }

    @Test
    @DisplayName("does not re-set the TTL on later requests, so the window cannot slide forever")
    void laterRequestsLeaveTtlAlone() throws Exception {
      // Given
      when(valueOperations.increment(anyString())).thenReturn(2L);

      // When
      filter.doFilter(
          guestRequest(GUEST_PATH), new MockHttpServletResponse(), new MockFilterChain());
    ...
    void ignoresForwardedForByDefault() throws Exception {
      ...
      verify(valueOperations).increment("ratelimit:guest:" + CLIENT_IP);
    }
    ...
    void usesForwardedForWhenTrusted() throws Exception {
      ...
      verify(valueOperations).increment("ratelimit:guest:198.51.100.99");
    }
  }
}
