...
echo "==> Đọc danh sách object key từ $(basename "$SQL_FILE")"

# Lấy mọi chuỗi dạng books/…, previews/…, covers/… trong file SQL.
grep -oE "(books|previews|covers)/[0-9]+/[A-Za-z0-9._-]+" "$SQL_FILE" | sort -u > "$STAGING/keys.txt"
